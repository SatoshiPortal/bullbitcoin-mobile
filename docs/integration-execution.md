...
The dependency check found two actual domain-to-data imports in our backup contents use cases. Both existed only to name `InspectVaultRecoveryPackage`, a function type defined beside a serialization codec. Moved that existing type beside `WalletBackupVaultPackageFacts`, removed the use cases' codec imports and adjusted the existing test harness import. No new file, type, layer, mapping or runtime operation was added. Also corrected the remote-inventory comment: this reader uses the app's backup identity; it is not the still-unimplemented cold-key discovery flow. All 222 wallet-backup tests pass after the relocation (`/home/francis/bbm-i8-backup-domain-boundary-tests.log`). The larger inventory builder's placement is unchanged; this is the narrow dependency correction, not an architecture-wide refactor.

Payjoin cancellation is committed separately as `e00515489`; app reconciliation is `e61d6babd`. Both passed their normal pre-commit hooks. The consolidated `make checks` run has passed its static/format stages and is now executing the workspace tests; the final total and exit status remain pending.

### I8 — verified durability and architecture-cleanup checkpoint

Cleanup `39370cd5f` passed normal commit hooks, including whole-project analysis, no Dart fixes and source formatting. `make checks` completed with exit 0 on this source: 3,393 root tests plus package counts 7/78/27/40/1/17/10, 3,573 total, zero failures (`/home/francis/bbm-i8-core-durability-checks.log`). The final check of the four backup/manifest/passphrase/settings domain trees found no imports into their data or legacy hexagonal directories after the function-type relocation. This does not assert that all existing code is architecturally perfect or that the full integration review is approved.

Current-source Android verification used only AVD `bullvault_integration_v2_20260909`, `emulator-5584`, the public mnemonic fixture and the existing local server/proxy on ports 8234/8235. Cleared the old development app data to accommodate the unreleased schema shape, then retained the server database across separate fresh installations. Publication passed in 33 seconds after a 75.8-second build (`/home/francis/bbm-i8-durable-current-android-publish.log`, APK SHA-256 `851823cf508f4dd69931660dfc34463481e04fec3ec4d77307d4ff435a671f17`). Recovery passed in 37 seconds after a 75.4-second build (`/home/francis/bbm-i8-durable-current-android-recover.log`, APK SHA-256 `a764b5f74b1559053d7030e8e73dec83ffe0e11b5439783b93d64054cd6ae529`). The test asserts the different adopted/fresh default-wallet references, restored inheritance-enabled descriptor, labels, remote inventory, recovery-fence clearance and successful republication. Test teardown removes each test installation; fixture data is reproducible and the local server backup remains retained. No real funds or public backup destination was used.

Rebuilt the normal `lib/main.dart` production-flavor x64 debug APK in 32.9 seconds and installed it successfully (`/home/francis/bbm-i8-durable-current-normal-build.log`, SHA-256 `bea8e245e1ec9878741e0612fcc8a1df691bc3561c4c79f280b73dbb90d11a58`). Android's `am start -W` reported a launch-wait timeout at 11 seconds, so that output alone is not claimed as a successful UI smoke test. A subsequent inspected capture shows the actual onboarding screen with its Next button (`/home/francis/bbm-i8-durable-current-normal.png`). The captured app-process log has skipped-frame warnings but no fatal exception, unhandled exception, Flutter caught exception, missing asset or missing-column marker (`/home/francis/bbm-i8-durable-current-normal-logcat.log`). This debug-emulator startup is not a production performance benchmark.
