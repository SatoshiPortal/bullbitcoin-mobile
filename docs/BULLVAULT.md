...
`recovery`, `inheritance`, and `lastResort` values. The complete version-1 example lives in
...
Saved recovery dates and recovery-file export are available offline. Preparing
new renewal dates requires a fresh time check. The user reviews the schedule,
confirms renewal and registration on the hardware devices, saves the new
recovery package, and registers it before activation. Setup can be left and
resumed later, but all
