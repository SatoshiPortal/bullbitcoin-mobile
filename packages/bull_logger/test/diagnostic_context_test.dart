...
void main() {
  test('runs probes concurrently and applies the per-probe timeout', () async {
    final source = _SlowDiagnosticContextSource();
    final stopwatch = Stopwatch()..start();
    final context = await DiagnosticContextProvider(
      source,
    ...
    expect(jsonDecode(context.toLogMessage()), containsPair('app', 'unknown'));
    expect(source.probesStarted, 6);
  });
...
final class _SlowDiagnosticContextSource implements DiagnosticContextSource {
  int probesStarted = 0;
  ...
  @override
  Future<String> appVersion() => _slow('version');

  @override
  Future<DiagnosticDeviceContext> device() =>
      _slow(const DiagnosticDeviceContext(platform: 'test', osVersion: 'test'));

  @override
  Future<int?> batteryLevel() => _slow(50);

  @override
  Future<List<String>> networkTypes() => _slow(const ['wifi']);

  @override
  Future<String> vpnStatus() => _slow('inactive');

  @override
  Future<DiagnosticResources> resources() => _slow(const DiagnosticResources());

  @override
  Future<DiagnosticTorContext> tor() =>
      _slow(const DiagnosticTorContext(state: 'ready'));
}
