...
import 'package:boltz_swaps/src/domain/entities/swap.dart' as swap_entity;
...
class BoltzDatasource {
  ...
  BoltzDatasource({
    ...
    // A bare host gets https (the production default cannot be downgraded);
    // an explicit http:// is a deliberate dev/staging opt-in (SSH tunnel to
    // a local backend) and the websocket follows it as ws://.
    _httpsUrl = _baseUrl.startsWith('http://') ? _baseUrl : 'https://$_baseUrl';
    _http = Dio(BaseOptions(baseUrl: _httpsUrl));
  }

  String get _wsBaseUrl => _baseUrl.startsWith('http://')
  ...
  Future<void> _processSwapEvent(
    ...
    try {
      ...
      await _boltzStore.mutate(swapId, () async {
        ...
        switch (mapping) {
          case SwapStale():
            // Row goes, key material stays: "no funds at risk" rests on a
            // null sendTxid plus the server's word, and a lockup broadcast
            // the instant before a crash would leave both wrong. An orphan
            // blob costs bytes; a deleted key would make the script-path
            // refund impossible forever.
            swapsLog.info(
              '[Boltz] deleting stale pending swap $swapId row '
              '(no funds at risk per server; secure blob retained)',
            );
            unsubscribeToSwaps([swapId]);
