...
Wallet synchronization uses a FIFO worker pool. `maxConcurrentJobs` must be greater than zero and bounds the number of eligible wallet callbacks running in one task; a newly available slot starts the next wallet without waiting for other workers. Returned failures and thrown exceptions do not stop later wallets from being attempted. The shell uses a limit of 2 for each chain task. Bitcoin and Liquid WorkManager tasks are distinct invocations, so this is a per-task bound rather than a constant total across both chains; runtime is not guaranteed to remain constant as the wallet count grows.
