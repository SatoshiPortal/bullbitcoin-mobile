...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(failure.outNetwork, SwapNetwork.lightning);
    },
  );
  ...
  test('publishes the provider-unavailable signal after HTTP 418', () async {
    when(
      () => remote.getBestSwapOption(
        amountSat: BigInt.from(1000),
        isInAmountFixed: false,
    ...
    final signal = expectLater(
      repository.watchSwapProviderUnavailable(),
      emits(true),
    );
    ...
    expect(repository.isSwapProviderUnavailable, isTrue);
    await signal;
  });
