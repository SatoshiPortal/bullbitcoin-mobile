    ...
    test(
      ...
      () async {
        ...
        for (final secret in ['zebra', 'hunter2', 'abandon']) {
          expect(failure.logMessage, isNot(contains(secret)), reason: secret);
        }
      },
...
void redactionOfStoredContent() {
  ...
  group('an Error is not a failure, but it is redacted all the same', () {
    // Rule 11: an `Error` is a programmer bug and keeps propagating to the
    // crash report — it must not be swallowed into a SecretFailure. But a
    // library's Error can quote its input (bdk, bip39, a FormatException
    // from `hex.decode`), so the message must not travel either. Type and
    // stack trace leave; the text does not.
    test('leaves the boundary with its type only', () async {
      await expectLater(
        boundary<void>(
