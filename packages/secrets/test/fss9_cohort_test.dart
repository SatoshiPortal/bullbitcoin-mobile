...
void main() {
  ...
  group('the entries are visible but unreadable', () {
    ...
    test('an entry that reads back empty is refused, not replaced', () async {
      // The plugin has been seen returning "" for an entry that exists. The
      // read path already calls that present-and-unreadable — the retry loop
      // treats it like null, then reports a read failure, never an absence.
      // The write path must agree, or the one write in this package lands on
      // exactly the entry the same class refuses to read. The module-key
      // namespace has held this since the start; this is its seed twin.
      final storage = FakeSecureStoragePlatform(
        entries: {'seed_$fingerprint': ''},
      );
      final secrets = secretsWith(storage);
      ...
      expect(
        storage.entries['seed_$fingerprint'],
        isEmpty,
