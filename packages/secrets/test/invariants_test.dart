...
void main() {
  ...
  test('the fluent sugar cannot do anything', () {
    ...
    final forwards =
        '_secret'.allMatches(body).length + '(this)'.allMatches(body).length;
  ...
  test('foreign dependencies are confined to the module that owns them', () {
    ...
    const owner = {
      'bull_sdk': {'crypto'},
      'recoverbull': {'crypto'},
      // `testing/` is a keystore too — the in-memory one the tests install.
      'flutter_secure_storage': {'data', 'testing'},
      // Flutter itself: the sealed widgets, and `PlatformException` — the
      // keystore plugin's error type, on both sides of the seam. `public/`,
      // `crypto/` and `domain/` stay plain Dart.
      'flutter': {'widgets', 'data', 'testing'},
    };
    ...
    for (final file in sources) {
      ...
      for (final dep in imports) {
        // Prefix match: `flutter_secure_storage_darwin` and its four federated
        // siblings are the keystore too, and belong where it does.
        // Longest key wins: `flutter_secure_storage_darwin` is the keystore, not Flutter.
        final allowed =
            (owner.entries
                    .where((o) => dep == o.key || dep.startsWith('${o.key}_'))
                    .toList()
                  ..sort((a, b) => b.key.length.compareTo(a.key.length)))
                .map((o) => o.value)
                .firstOrNull;
        if (allowed != null && !allowed.contains(mine)) {
          offenders.add(
            '${file.path} imports $dep (belongs to ${allowed.join('/, ')}/)',
          );
        }
  ...
  test('the testing library never reaches production code', () {
    // `package:secrets/testing.dart` installs an in-memory keystore
    // process-wide. It is a second public library, outside `src/`, so
    // `implementation_imports` does not cover it — this does. Only the
    // seam's own files may name it; nothing under `secrets.dart` may.
    final offenders = <String>[];
    for (final file in sources) {
      if (moduleOf(file) == 'testing' || file.path == 'lib/testing.dart') {
        continue;
      }
      final source = code(read(file.path));
      if (RegExp(
        r"(import|export) 'package:secrets/(testing\.dart|src/testing/)",
      ).hasMatch(source)) {
