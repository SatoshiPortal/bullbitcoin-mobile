...
final class Secrets {
  ...
  Future<Result<Secret, SecretFailure>> generate({int wordCount = 12}) async =>
  ...
  ///
  /// ⚠️ **A vault carries the words alone**, so pass [passphrase] to restore the wallet the user actually had. Without it the result is a [WordsOnly] — restored, storable, usable, and a different Bitcoin wallet if there ever was a passphrase. See the README, § Passphrase.
  ...
  Future<Result<void, SecretFailure>> resetDatabaseKey({
    required String package,
    required String name,
  }) => _databaseKeys.reset(package: package, name: name);
