...
///
/// **This class is the audit surface.** Every operation the package can perform on a secret is a method here, flat, in one file — so the four that hand back material ([revealWords], [bip85Hex], [bip85Mnemonic], [swapKey]) are read in the same pass as the ones that do not. The grouped spelling — `secret.sign.psbt(…)` — is sugar over these methods and holds no behaviour; see `extensions.dart`, whose inertness a test asserts.
///
...
final class Secret {
  ...
  @internal
  Future<Result<RevealedMnemonic, SecretFailure>> revealWords({
