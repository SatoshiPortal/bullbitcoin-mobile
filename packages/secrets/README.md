...
**The stored mnemonic has no exit.** `Secret.revealWords` is `@internal`:
...
| material leaves at four named methods | the set of `Secret` methods returning material is exactly `{revealWords, bip85Hex, bip85Mnemonic, swapKey}` — and `revealWords` is `@internal` |
