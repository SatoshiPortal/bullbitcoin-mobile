...
- **Backup vault** — `encryptVault()` → `({EncryptedVault vault, VaultKey vaultKey})`
...
Display is **not** a handle method (the handle returns data, never words/bytes). It is a single polymorphic sealed widget that does all conditional rendering from the secret itself — words grid (with the passphrase row only when one is stored) for a mnemonic, hex for a seed, plus unavailable/locked states. Every branch is `PrivacyGuard`-wrapped and reads via the internal use-and-forget path.
