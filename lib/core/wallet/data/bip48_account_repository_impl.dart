...
final class Bip48AccountRepositoryImpl implements Bip48AccountRepository {
  ...
  Future<Set<int>> _readAccounts({
    ...
    final normalizedSeedFingerprint = seedFingerprint.toLowerCase();
    var changed = false;
    for (final usage in usages) {
      if (usage.coinType != coinType || accounts.contains(usage.account)) {
        continue;
      }
      final localSeedFingerprint = usage.localSeedFingerprint?.toLowerCase();
      if (localSeedFingerprint != null) {
        if (localSeedFingerprint == normalizedSeedFingerprint) {
          changed = accounts.add(usage.account) || changed;
        }
        continue;
      }
      if (usage.seedFingerprint.toLowerCase() != normalizedSeedFingerprint) {
        continue;
      }
