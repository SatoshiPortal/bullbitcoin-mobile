...
class DiagnosticContextProvider {
  ...
  const DiagnosticContextProvider(this._source);

  Future<DiagnosticContext> load() async {
    final app = await _bestEffort(_source.appVersion);
    final device = await _bestEffort(_source.device);
    final resources = await _bestEffort(_source.resources);
