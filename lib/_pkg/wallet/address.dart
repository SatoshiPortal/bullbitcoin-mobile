  ...
  Future<(Wallet?, Err?)> newAddress(Wallet wallet) async {
    try {
      ...
      switch (wallet.baseWalletType) {
        ...
        case BaseWalletType.Liquid:
          ...
          final (addr, errAddr) = await _lwkAddress.peekIndex(
            liqWallet!,
            wallet.lastGeneratedLiqAddress!.index! + 1,
          );
          if (errAddr != null) throw errAddr;
          address = addr!;
      ...
      final w = updatedWallet.copyWith(
        lastGeneratedAddress: wallet.baseWalletType == BaseWalletType.Bitcoin
            ? addr
            : updatedWallet.lastGeneratedAddress,
        lastGeneratedLiqAddress: wallet.baseWalletType == BaseWalletType.Liquid
            ? addr
            : updatedWallet.lastGeneratedLiqAddress,
      );
