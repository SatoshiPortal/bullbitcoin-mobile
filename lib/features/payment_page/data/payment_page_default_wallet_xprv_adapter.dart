...
class PaymentPageDefaultWalletXprvAdapter
    implements PaymentPageDefaultWalletXprvPort {
  final GetSettingsUsecase _getSettings;
  final WalletRepository _walletRepository;
  final SeedRepository _seedRepository;

  const PaymentPageDefaultWalletXprvAdapter({
  ...
  Future<String> deriveDefaultWalletXprv() async {
    ...
    if (wallets.isEmpty) {
      throw const PaymentPageException.noDefaultBitcoinWallet();
    }
    // Fail loud if more than one default bitcoin wallet exists: the identity is
    // derived from THE default wallet, so an ambiguous set must not silently
    // bind to an arbitrary one. Matches the prepare path's `.single`.
