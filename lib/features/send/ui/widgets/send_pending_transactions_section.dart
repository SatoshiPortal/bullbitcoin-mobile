...
class SendPendingTransactionsSection extends StatelessWidget {
  const SendPendingTransactionsSection({super.key});

  @override
  Widget build(BuildContext context) {
    final transactions = context.select(
      (SendPendingTransactionsCubit cubit) => cubit.state.transactions,
    );
    final invalidCount = context.select(
      (SendPendingTransactionsCubit cubit) => cubit.state.invalidCount,
    );
...
class _PendingTransactionTile extends StatelessWidget {
  ...
  Future<void> _delete(BuildContext context) async {
    ...
    if (confirmed == true && context.mounted) {
      final deleted = await context.read<SendPendingTransactionsCubit>().delete(
        transaction,
      );
