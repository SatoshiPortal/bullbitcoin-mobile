...
class RecoverBullBloc extends Bloc<RecoverBullEvent, RecoverBullState> {
    ...
    try {
      ...
      switch (await _fetchVaultKeyWithStatusFromServerUsecase.execute(
        ...
        case Err(:final failure):
          ...
          if (failure is core.KeyServerRateLimitedFailure) {
            unawaited(
  ...
  RecoverBullFailure _fetchKeyFailure(core.RecoverBullCoreFailure failure) =>
      switch (failure) {
        ...
        // 503 and plain unreachability are service issues, never an attack:
        // surface them as a fetch failure, never as a lockout alarm.
        core.KeyServerBusyFailure() => const VaultKeyFetchFailure(),
  ...
  RecoverBullFailure _storeKeyFailure(
    ...
    core.KeyServerBusyFailure() => const KeyServerConnectionFailure(),
