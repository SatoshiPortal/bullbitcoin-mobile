...
/// Switches a product back to Bitcoin-only (percentage 0). Requires no scoped
/// key, no currency, and no eligibility preflight, and stays available even
/// when the server is not accepting new settings.
class DisableFiatSettlementUsecase {
  final BullnymFacade _bullnym;
  final FiatSettlementDefaultWalletXprvPort _xprvPort;
  final NostrIdentityFacade _nostrIdentity;

  const DisableFiatSettlementUsecase({
  ...
  execute({required FiatSettlementProduct product}) async {
    ...
    final result = await _bullnym.setFiatSettlement(
      signer: signer,
