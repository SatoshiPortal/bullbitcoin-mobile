...
mixin SpBackendFormMixin<S extends SpBackendFormState<S>> on Cubit<S> {
  ...
  Future<void> fetchRegtestDefaults() async {
    ...
    await _landDefaults(BitcoinNetwork.regtest);
  }
  ...
  Future<void> _landDefaults(BitcoinNetwork network) async {
  ...
  Future<void> _testBackend(SpBackendKind kind) async {
    ...
    if (url.isEmpty || isClosed) return;
    emit(
      state.withForm(
        state.form.applyConnectionStatus(
          kind,
          SpConnectionStatus.testing,
          null,
        ),
      ),
    );
    final result = await backendTestUsecase.execute(kind, url);
    ...
    switch (result) {
      case Ok():
        emit(
          state.withForm(
            state.form.applyConnectionStatus(kind, SpConnectionStatus.ok, null),
          ),
        );
      case Err(:final failure):
        emit(
