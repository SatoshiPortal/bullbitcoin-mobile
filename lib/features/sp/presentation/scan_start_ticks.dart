...
List<ScanStartTick> scanStartTicks({
  ...
  // Newest-first within each unit: 1 year, then 6 down to 1 months, then 3
  // down to 1 weeks.
  final offsets = <({_TickUnit unit, int count})>[
    (unit: _TickUnit.year, count: 1),
    for (var months = 6; months >= 1; months--)
      (unit: _TickUnit.month, count: months),
    for (var weeks = 3; weeks >= 1; weeks--)
      (unit: _TickUnit.week, count: weeks),
  ];
