...
class CreateSpWalletUsecase {
  ...
  Future<Result<void, SpFailure>> execute({
    ...
    // up; refuse a second setup. A corrupt config counts as absent (it will be
    // overwritten by this setup).
    ...
    // "succeed" yet be unreachable, since loads are blocked while the sentinel
    // exists.
    if (hasSentinel) {
      try {
