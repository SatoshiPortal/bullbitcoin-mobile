...
class SpNotificationsWatcher {
  ...
  Future<void> _establishAndResubscribe() async {
    try {
      // A genuinely revoked / not-set-up wallet reports Ok(null); do NOT
      // re-subscribe (a dead session's stream would close at once and loop).
      // A failed establish is not a revoke, but re-subscribing over it would
      // loop just the same, so both stop here.
      final result = await _ensureSpSessionUsecase.execute();
      final controller = _controller;
      if (controller == null || controller.isClosed) return;
      switch (result) {
        case Err(:final failure):
          log.warning(
            'SpNotificationsWatcher: re-establish failed: '
            '${failure.logMessage}',
          );
          return;
        case Ok(value: null):
