            ...
            BullButton.big(
              label: switch (state.step) {
                BullVaultRenewalStep.review =>
                  state.timeReference != null
                      ? context.loc.bullVaultRenew
                      : state.failure != null
                      ? context.loc.retry
                      : context.loc.bullVaultReviewRenewal,
              ...
              onPressed: switch (state.step) {
                BullVaultRenewalStep.review =>
                  state.timeReference == null
                      ? cubit.prepareRenewal
                      : () => _confirmRenewal(context),
                BullVaultRenewalStep.recoveryPackage ||
              ...
              disabled: switch (state.step) {
                BullVaultRenewalStep.review =>
                  state.isRenewing || state.isPreparingRenewal,
                BullVaultRenewalStep.recoveryPackage ||
...
final class _RenewalReview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final currentActivations = <({String label, DateTime date})>[
      ...
      if (policy.lastResortActivationTimestamp case final activationTimestamp?)
        (
    ...
    final newActivations = <({String label, DateTime date})>[
      if (reference != null) ...[
        ...
        if (schedule.lastResortActivationDate(reference) case final date?)
          (label: context.loc.bullVaultLastResortDelay, date: date),
      ],
    ]..sort((first, second) => first.date.compareTo(second.date));
    return Column(
      crossAxisAlignment: .stretch,
      children: [
        ...
        BullVaultScheduleFields(
          ...
          onLastResortChanged: (value) => context
              .read<BullVaultRenewalCubit>()
              .updateSchedule(schedule.copyWith(lastResortDelay: value)),
        ),
        if (reference != null) ...[
          const Gap(24),
          Text(
...
final class _CurrentRecoveryExport extends StatelessWidget {
  const _CurrentRecoveryExport();
          ...
          if (previous.hasFunds) ...[
            ...
            BullButton.big(
              ...
              onPressed: () async {
                ...
                if (context.mounted) {
                  await context.read<BullVaultRenewalCubit>().refreshDetails();
                }
              },
              bgColor: context.appColors.primary,
              textColor: context.appColors.onPrimary,
