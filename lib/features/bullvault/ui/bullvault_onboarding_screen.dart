...
final class BullVaultOnboardingScreen extends StatelessWidget {
  ...
  Widget _stepContent(
    ...
    BullVaultOnboardingStep.recoveryPackage => BullVaultRecoveryPackageStep(
      descriptor: state.result!.policy.descriptor,
...
String _accountFingerprint(String accountKey) {
...
Future<void> _acquireInheritanceMnemonic(
  ...
  final accountKey = await showBullVaultInheritanceMnemonic(
    context,
    network: network,
    generated: generated,
  );
    ...
    return Column(
      ...
      children: [
        ...
        _SummaryTile(
          ...
          secondaryBody:
              state.everydayKeySource == BullVaultEverydayKeySource.bullMobile
              ? null
              : _accountFingerprint(state.everydayInput),
        ),
        ...
        _SummaryTile(
          ...
          secondaryBody: _accountFingerprint(state.coldInput),
        ),
        if (state.usesTwoColdKeys) ...[
          const Gap(12),
          _SummaryTile(
            ...
            secondaryBody: _accountFingerprint(state.secondColdInput),
          ),
        ],
        if (state.includeInheritance) ...[
          ...
          _SummaryTile(
            ...
            secondaryBody: _accountFingerprint(state.inheritanceInput),
          ),
        ],
        const Gap(12),
        _SummaryTile(
        ...
        if (state.schedule.lastResortActivationDate(referenceTime)
            case final date?) ...[
          const Gap(12),
          _SummaryTile(
            icon: Icons.restore,
            title: context.loc.bullVaultLastResortRecovery,
            body: context.loc.bullVaultDelayValue(
              _delay(context, state.schedule, state.schedule.lastResortDelay!),
            ),
            secondaryBody: _recoveryDate(context, date),
          ),
          const Gap(12),
          BullInfoCard(
            description: context.loc.bullVaultLastResortDescription,
            tagColor: context.appColors.warning,
            bgColor: context.appColors.warningContainer,
          ),
          if (state.passphraseFreeRecovery) ...[
            const Gap(8),
            Text(context.loc.bullVaultLastResortPassphraseFreeDescription),
          ],
        ],
        if (state.usesTwoColdKeys && !state.includeInheritance) ...[
          const Gap(16),
            ...
            children: [
              ...
              if (state.includeInheritance) ...[
                ...
                _AdvancedSwitchRow(
                  ...
                  title: context.loc.bullVaultLastResortRecovery,
                  description: context.loc.bullVaultLastResortDescription,
                  value: state.schedule.lastResortDelay != null,
                  onChanged: cubit.setLastResortRecovery,
                ),
                if (state.passphraseFreeRecovery) ...[
                  const Gap(8),
                  Text(
                    context.loc.bullVaultLastResortPassphraseFreeDescription,
