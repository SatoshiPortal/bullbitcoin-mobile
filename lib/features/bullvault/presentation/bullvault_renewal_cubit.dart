...
final class BullVaultRenewalCubit extends Cubit<BullVaultRenewalState> {
  ...
  String? get currentRecoveryPackageContent {
    final record = state.details?.record;
    return record == null
        ? null
        : _encodeBullVaultRecoveryPackageUsecase.execute(
            record.recoveryPackage,
          );
  }

  Future<void> prepareRenewal() async {
    final details = state.details;
    if (details == null || state.isPreparingRenewal || state.renewal != null) {
      return;
    }
    emit(state.copyWith(isPreparingRenewal: true, clearFailure: true));
    final result = await _prepareTimeReferenceUsecase.execute(
      isTestnet: details.policy.network.isTestnet,
    );
    if (isClosed) return;
    switch (result) {
      case Ok(:final value):
        emit(state.copyWith(timeReference: value, isPreparingRenewal: false));
      case Err(:final failure):
        emit(state.copyWith(failure: failure, isPreparingRenewal: false));
    }
  }
