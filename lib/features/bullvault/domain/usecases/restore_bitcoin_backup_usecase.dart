...
final class RestoreBitcoinBackupUsecase {
  final BitcoinDescriptorBackupRepository _repository;
  const RestoreBitcoinBackupUsecase(this._repository);
  @useResult
  Future<Result<RestoredBackupWallet, BullVaultFailure>> execute({
    required String descriptor,
    required BitcoinBackupNetwork network,
    required ElectrumConnection connection,
    required DescriptorBackupSession session,
  }) => _repository.restore(descriptor, network, connection, session);
}
