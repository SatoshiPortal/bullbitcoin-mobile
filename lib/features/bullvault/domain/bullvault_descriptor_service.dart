      ...
      for (final ordered in _permutations(grouped.values.toList())) {
        try {
          ...
          final generation = _generationFor(
            ...
            includesLastResort: shape.lastResort != null,
          ...
          final policy = BullVaultPolicy.restoreDescriptor(
            ...
            lastResortActivationTimestamp: shape.lastResort,
          );
          if (_matchesPolicyDescriptor(policy, actual: parsed.descriptor)) {
            matches.add(policy);
          }
        } on ArgumentError {
          continue;
  ...
  bool matchesPolicyDescriptor(BullVaultPolicy policy) {
    try {
      final actual = _descriptorPort
          ...
          .descriptor;
      return _matchesPolicyDescriptor(policy, actual: actual);
    } on Exception {
      return false;
    }
  }

  bool _matchesPolicyDescriptor(
    BullVaultPolicy policy, {
  ...
  int? _generationFor({
    ...
    final generations = <int?>[
      _generationForSigner(
        everyday,
        occurrencesPerGeneration: delayedMobileRecovery == null
            ? (includesLastResort ? 3 : 2)
            : 1,
      ),
      if (delayedMobileRecovery != null)
        _generationForSigner(
          delayedMobileRecovery,
          occurrencesPerGeneration: includesLastResort ? 2 : 1,
        ),
