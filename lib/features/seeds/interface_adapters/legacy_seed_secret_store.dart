...
class LegacySeedSecretStore implements LegacySeedSecretStorePort {
  final FlutterSecureStorage _flutterSecureStorage;

  LegacySeedSecretStore({required FlutterSecureStorage flutterSecureStorage})
  ...
  Future<List<SeedSecret>> loadAll() async {
    try {
      ...
      List<SeedSecret> parseLegacySeedsInIsolate(
        Map<String, String> allEntries,
      ) {
        final secrets = <SeedSecret>[];
          ...
          try {
            ...
            if (json.containsKey('passphrases') &&
              ...
              if (processedPassphrases.isEmpty) {
                secrets.add(
                  SeedMnemonicSecret(words: mnemonicWords, passphrase: null),
                );
              }
            } else {
