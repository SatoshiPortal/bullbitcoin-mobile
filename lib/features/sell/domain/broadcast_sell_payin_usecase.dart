...
class BroadcastSellPayinUsecase {
  ...
  Future<Result<String, SellFailure>> bitcoin(
    ...
    try {
      ...
      log.severe(
        message: 'Failed to broadcast the Bitcoin sell payin',
        error: e,
        trace: st,
      );
  ...
  @useResult
  Future<Result<String, SellFailure>> liquid(String signedPset) async {
    try {
      return Ok(await _broadcastLiquidTransactionUsecase.execute(signedPset));
    } catch (e, st) {
      log.severe(
        message: 'Failed to broadcast the Liquid sell payin',
        error: e,
        trace: st,
      );
      return Err(SellUnexpectedFailure('$e'));
