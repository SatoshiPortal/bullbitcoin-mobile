...
class GetTransactionsUsecase {
  ...
  Future<Result<List<Transaction>, TransactionFailure>> execute({
    ...
    try {
      ...
      // Returned, never thrown: both callers switch on the result and have no
      // try/catch, so a throw here would strand the list on "syncing" and the
      // export screen on "loading". The raw reason stays in the log above.
      return Err(
        TransactionAggregationFailure('aggregation failed: ${e.runtimeType}'),
      );
    }
  }
