...
void main() {
  ...
  final fakeBalance = SpBalance(
    confirmedSat: Sats.fromInt(10000),
    totalUnifiedSat: Sats.fromInt(10000),
  );
  ...
  test('totalBalance getter returns totalUnifiedSat from balance', () async {
    await cubit.load();
    expect(cubit.state.totalBalance, Sats.fromInt(10000));
  });
  ...
  test('a progress with no prior started still shows a moving bar', () async {
    // The header sync starts at launch, so a subscriber joining mid-run sees a
    // Progress first. Without an origin the bar rendered 0 for the whole run.
    ...
    expect(cubit.state.headerValidationFrom, 850000);
    expect(cubit.state.headerValidationProgress, greaterThan(0.0));
  });
