...
void main() {
  ...
  test(
    'ScanStarted notification sets isScanning=true and scan bounds',
    () async {
      await cubit.load();
  ...
  test(
    ...
    () async {
      ...
      notifController.add(const SpScanReceiveProgress(810000, 850000));
      await Future.delayed(Duration.zero);
  ...
  test('ScanSpendProgress switches to step 2 and rebases the bar', () async {
    ...
    notifController.add(const SpScanStarted(800000, 850000));
    await Future.delayed(Duration.zero);
    notifController.add(const SpScanReceiveProgress(850000, 850000));
    await Future.delayed(Duration.zero);
    ...
    notifController.add(const SpScanSpendProgress(800000, 850000));
    await Future.delayed(Duration.zero);
    ...
    expect(cubit.state.scanCurrent, 800000);
    expect(cubit.state.scanTo, 850000);
    // Inclusive: the first block of the rebased range already counts, so a
    // just-entered phase reads just above zero rather than flat 0.
    expect(cubit.state.scanProgress, closeTo(0.0, 0.001));
    expect(cubit.state.scanProgress, greaterThan(0));

    notifController.add(const SpScanSpendProgress(825000, 850000));
    await Future.delayed(Duration.zero);
    expect(cubit.state.scanProgress, closeTo(0.5, 0.001));
  });

  test(
    'ScanCompleted notification sets isScanning=false and reloads data',
    () async {
      await cubit.load();
  ...
  test(
    'ScanStopped notification sets isScanning=false and reloads data',
    () async {
      await cubit.load();
      clearInteractions(loadUsecase);
  ...
  test(
    'ScanFailed notification sets isScanning=false, sets error, reloads data',
    () async {
      await cubit.load();
      clearInteractions(loadUsecase);

      notifController.add(const SpScanFailed(SpUnexpected('network error')));
      await Future.delayed(Duration.zero);
  ...
  test('coin notification during scan defers reload to ScanCompleted', () async {
    ...
    notifController.add(SpNewOutput('o:0', Sats.fromInt(1)));
    await Future.delayed(Duration.zero);
  ...
  test('scanProgress getter returns correct fraction', () async {
    ...
    notifController.add(const SpScanReceiveProgress(50, 100));
    await Future.delayed(Duration.zero);

    // Inclusive range [0, 100]: block 50 is the 51st of 101.
    expect(cubit.state.scanProgress, closeTo(51 / 101, 0.001));
  });
  ...
  group('async edge cases', () {
    ...
    test('a scanner-already-running error does not clear isScanning', () async {
    ...
    test('a non-busy scan error still clears isScanning', () async {
      when(
        () => scanUsecase.execute(startHeight: any(named: 'startHeight')),
      ).thenAnswer((_) async {
        notifController.add(const SpScanStarted(1, 10));
        await Future.delayed(Duration.zero);
        return const Err<void, SpFailure>(SpUnexpected('backend unreachable'));
      });
      await cubit.load();
    ...
    test(
      'stopScan() logs an Err without surfacing it as an error state',
      () async {
        when(() => stopUsecase.execute()).thenAnswer(
          (_) async => const Err<void, SpFailure>(SpUnexpected('stop failed')),
        );
        await cubit.load();

        // Best-effort: the scan tears down async, so a failed stop is not the
        // user's problem.
        await cubit.stopScan();
    ...
    test(
      'stopScan() lets a thrown bug propagate instead of hiding it',
      () async {
        when(() => stopUsecase.execute()).thenThrow(StateError('stop failed'));
        await cubit.load();

        await expectLater(cubit.stopScan(), throwsStateError);
        expect(cubit.state.error, isNull);
      },
    );

    test(
      'a notification arriving after close() does not throw or emit',
      () async {
        await cubit.load();
    ...
    test(
      'self-heals when its notification stream closes: the watcher '
      ...
      () async {
        ...
        ).thenAnswer((_) async => Ok(buildData().wallet));
        ...
        // Subscribing to the fresh stream is the deterministic re-establish
        // signal, so the wait does not depend on machine load.
        final resubscribed = Completer<void>();
        when(() => watchUsecase.execute()).thenAnswer((_) {
          if (!resubscribed.isCompleted) resubscribed.complete();
          return freshController.stream;
        });
        ...
        // reloaded the wallet data once. Pinned exactly, so a self-heal loop
        // firing repeatedly fails here instead of passing.
        verify(() => loadUsecase.execute()).called(2);
