...
void main() {
  ...
  testWidgets('backgrounding while privacy setup is pending never derives', (
    ...
    await tester.pumpAndSettle();
    expect(revealCalls, 0);
    // The dialog releases protection from both the abandoned reveal and
    // dispose; `PrivacyScreen` is idempotent, so the platform sees one
    // `screenshotOn` for the two releases.
    expect(privacyOffCalls, 1);
