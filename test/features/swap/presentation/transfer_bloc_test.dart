...
class _MockCalculateBitcoin extends Mock implements CalculateBitcoinAbsoluteFeesUsecase {}
class _MockCalculateLiquid extends Mock implements CalculateLiquidAbsoluteFeesUsecase {}
...
class _MockBroadcastBitcoin extends Mock implements BroadcastBitcoinTransactionUsecase {}
class _MockBroadcastLiquid extends Mock implements BroadcastLiquidTransactionUsecase {}
class _MockVerifyChain extends Mock implements VerifyChainSwapAmountSendUsecase {}
...
class _MockConvertSats extends Mock implements ConvertSatsToCurrencyAmountUsecase {}
class _MockPreviewFee extends Mock implements PreviewBitcoinFeeUsecase {}
class _MockPreviewPresets extends Mock implements PreviewBitcoinFeePresetsUsecase {}
class _MockCheckConsolidation extends Mock implements CheckLiquidConsolidationUsecase {}
...
void main() {
  ...
  setUp(() {
    ...
    bloc = TransferBloc(
      ...
      getWalletsUsecase: _MockGetWallets(),
      ...
      convertSatsToCurrencyAmountUsecase: _MockConvertSats(),
  ...
  test('broadcasts a prepared order swap and emits its transaction id', () async {
    final broadcasting = _prepared(status: OrderSwapLocalStatus.broadcastUnknown);
    ...
    final completed = bloc.stream.firstWhere((state) => state.txId == 'txid-1');
    bloc.add(const TransferEvent.confirmed());
    await completed;
    await subscription.cancel();
    verify(() => markUnknown.execute('local-1')).called(1);
    verify(() => broadcastBitcoin.execute('signed-psbt', isPsbt: true)).called(1);
    verify(() => markBroadcast.execute(localId: 'local-1', transactionId: 'txid-1')).called(1);
...
ChainSwap _swap() => Swap.chain(
  id: 'swap-1', keyIndex: 0, type: SwapType.bitcoinToLiquid,
  status: SwapStatus.pending, environment: Environment.testnet,
  creationTime: DateTime.utc(2026), sendWalletId: 'wallet-1',
  paymentAddress: 'payin-address', paymentAmount: 1000,
) as ChainSwap;

Wallet _wallet() => Wallet(
  origin: 'wallet-1', network: Network.bitcoinTestnet, xpubFingerprint: 'fingerprint',
  scriptType: ScriptType.bip84, xpub: 'xpub', externalPublicDescriptor: 'external',
  internalPublicDescriptor: 'internal', signer: SignerEntity.local, signerDevice: null,
