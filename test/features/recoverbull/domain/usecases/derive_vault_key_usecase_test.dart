...
void main() {
  // Independently checked with Node crypto: BIP39 PBKDF2 -> BIP32 CKDpriv at
  // 83696968'/1608'/0'/586053381['] -> HMAC-SHA512('bip-entropy-from-k', k).
  // The legacy, unhardened last index really is a DIFFERENT key. Do not
  // normalize that apostrophe or generate expectations using the code below.
  const expectedKey =
  ...
  for (final (path, key) in [
    ...
    test(
      ...
      () async {
        final result = await usecase.execute(backup(path: path, key: key));
  ...
  for (final path in [
    ...
    test(
      ...
      () async {
        final result = await usecase.execute(backup(path: path));
        expect(
  ...
  test(
    ...
    () async {
      final result = await usecase.execute(backup(path: "1608'/0'/1'"));
      expect(
        (result as Err<String, RecoverBullFailure>).failure,
  ...
  test(
    ...
    () async {
      ...
      final result = await usecase.execute(backup());
  ...
  test(
    ...
    () async {
      final result = await usecase.execute(backup(payload: '{"metadata": {}}'));
      expect(
        (result as Err<String, RecoverBullFailure>).failure,
        isA<VaultLocalKeyMismatchFailure>(),
      );
