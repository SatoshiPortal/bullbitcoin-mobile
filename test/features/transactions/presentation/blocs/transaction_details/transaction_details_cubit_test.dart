...
void main() {
  ...
  group('TransactionDetailsCubit.broadcastPayjoinOriginalTx guard', () {
    ...
    test('a later successful load clears a failed broadcast', () async {
      // The broadcast button renders this failure inline, so without a clear
      // on the success path the error stays under a session that has since
      // moved on.
      final payjoin = _receiver();
      when(() => getPayjoinById.execute(payjoin.id)).thenAnswer(
        (_) async => Ok<PayjoinSession, TransactionFailure>(payjoin),
      ...
      when(() => broadcastOriginalTransaction.execute(any())).thenAnswer(
        (_) async => const Err<PayjoinSession, TransactionFailure>(
          TransactionPayjoinBroadcastFailure('node rejected'),
        ),
      );
      ...
      expect(cubit.broadcastPayjoinOriginalTx(), isTrue);
      await pumpEventQueue();
      expect(cubit.state.payjoinBroadcastFailure, isNotNull);

      // A watcher-driven reload of the same session.
      await cubit.initByPayjoinId(payjoin.id);

      expect(cubit.state.payjoinBroadcastFailure, isNull);
    });
