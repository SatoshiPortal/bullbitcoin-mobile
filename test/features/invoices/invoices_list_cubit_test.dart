...
void main() {
  ...
  test('load fetches with NO server status filter and marks loaded', () async {
    ...
    final sent = verify(() => facade.list(captureAny())).captured.single
        as ListInvoicesCommand;
    expect(sent.status, isNull); // no server-side filter
    await cubit.close();
  });

  test('the status filter is applied CLIENT-SIDE over the loaded set', () async {
    when(() => facade.list(any())).thenAnswer(
      (_) async => ListInvoicesResult(
        invoices: [
  ...
  test('an empty result yields the honest empty state (no error)', () async {
    when(() => facade.list(any())).thenAnswer(
      (_) async => const ListInvoicesResult(
        invoices: [],
        page: 1,
        pageSize: 100,
        hasMore: false,
  ...
  test('a facade error flips to the error state with the typed failure',
      () async {
    when(() => facade.list(any()))
        .thenThrow(const InvoicesException.network());

    final cubit = InvoicesListCubit(facade: facade);
    await cubit.load();

    expect(cubit.state.status, InvoicesListStatus.error);
    expect(cubit.state.failure?.kind, InvoicesErrorKind.network);
    await cubit.close();
  });
}
