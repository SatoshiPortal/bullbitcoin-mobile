...
void main() {
  ...
  test(
    'resolves the signer then delegates, returning the final status',
    () async {
      when(
  ...
  test(
    'ownership rejection surfaces as notFound (server 404-as-ownership)',
    () async {
      when(
        () => payService.cancelInvoice(
          signer: any(named: 'signer'),
      ...
      final result = await usecase.execute(
        CancelInvoiceCommand(invoiceId: InvoiceId('foreign')),
      );
      expect(
        result,
        isA<Err<CancelInvoiceResult, InvoicesFailure>>().having(
  ...
  test(
    'a benign already-terminal cancel returns its pre-existing status',
    () async {
      when(
        () => payService.cancelInvoice(
          signer: any(named: 'signer'),
