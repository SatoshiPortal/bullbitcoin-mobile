...
void main() {
  ...
  test('sats submit builds an amountSat command and exposes the share URL',
      () async {
    when(() => facade.create(any())).thenAnswer((_) async => result);
    ...
    expect(cubit.state.result?.shareUrl.value,
  ...
  test('fiat submit converts to minor units by the currency precision',
      () async {
    when(() => facade.create(any())).thenAnswer((_) async => result);
  ...
  test('no rail selected is refused locally with zero wire calls', () async {
    ...
    expect(cubit.state.failure?.kind, InvoicesErrorKind.invalidInput);
    verifyNever(() => facade.create(any()));
    await cubit.close();
  });

  test('a facade failure surfaces without leaving the submitting state',
      () async {
    when(() => facade.create(any()))
        .thenThrow(const InvoicesException.noDefaultLiquidWallet());
    ...
    expect(cubit.state.failure?.kind, InvoicesErrorKind.noDefaultLiquidWallet);
    expect(cubit.state.isSubmitted, isFalse);
    await cubit.close();
  });

  test('double-submit is inert: the second submit does not re-hit the facade',
      () async {
    var calls = 0;
