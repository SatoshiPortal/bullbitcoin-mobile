...
class _MockWatchDetails extends Mock implements WatchBullVaultDetailsUsecase {}
...
void main() {
  ...
  test(
    'refreshes predecessor balances without resetting the selected schedule',
    () async {
      final load = _MockLoadRenewal();
      final watchDetails = _MockWatchDetails();
      final watchMigration = _MockWatchMigration();
      final updates =
          StreamController<Result<BullVaultDetails?, BullVaultFailure>>();
      var details = _details();
      when(() => load.execute(details.record.walletId)).thenAnswer(
        (_) async => Ok(BullVaultRenewalLoad(details: details, renewal: null)),
      );
      when(
        () => watchDetails.execute(details.record.walletId),
      ...
      cubit.updateSchedule(schedule);
      details = _details(balanceSat: 1000);
      updates.add(Ok(details));
      await pumpEventQueue();
      expect(
        cubit.state.details!.previousVaults.single.wallet.balanceSat,
        BigInt.from(1000),
      );
      expect(cubit.state.schedule, schedule);
      details = _details();
      await cubit.refreshDetails();
      expect(
        cubit.state.details!.previousVaults.single.wallet.balanceSat,
        BigInt.zero,
      );
  ...
  test(
    'retries recovery-date review, then confirms and activates a renewal',
    () async {
      final load = _MockLoadRenewal();
      ...
      when(
        () => load.execute(details.record.walletId),
      ).thenAnswer((_) async => Ok(BullVaultRenewalLoad(details: details)));
      var timeRequests = 0;
      when(
        () => prepareTime.execute(isTestnet: details.policy.network.isTestnet),
      ).thenAnswer(
        (_) async => ++timeRequests == 1
            ? const Err(BullVaultClockMismatchFailure())
            : Ok(reference),
      );
      ...
      when(
        () => encode.execute(details.record.recoveryPackage),
      ).thenReturn('current-package');
      when(
        () => watchMigration.execute(
          previousWalletId: 'retired-wallet',
          migrationAddress: details.migrationAddress!,
      ...
      await cubit.prepareRenewal();
      expect(cubit.state.failure, isA<BullVaultClockMismatchFailure>());
      expect(cubit.state.details, same(details));
      expect(cubit.state.isPreparingRenewal, isFalse);
      expect(cubit.currentRecoveryPackageContent, 'current-package');
      await cubit.renew(label: 'Renewed BullVault');
      verifyNever(() => renew.execute(any()));
      await cubit.prepareRenewal();
      ...
      expect(cubit.state.step, BullVaultRenewalStep.recoveryPackage);
      final request =
          verify(() => renew.execute(captureAny())).captured.single
...
BullVaultDetails _details({int balanceSat = 0}) {
    ...
    previousVaults: [
      BullVaultPreviousVault(
        ...
        wallet: _wallet(
          'retired-wallet',
          isHidden: true,
        ).copyWith(balanceSat: BigInt.from(balanceSat)),
      ),
    ],
  );
}
