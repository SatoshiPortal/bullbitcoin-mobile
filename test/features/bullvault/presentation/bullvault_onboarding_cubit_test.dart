...
void main() {
  ...
  test('uses the selected profile schedule', () async {
    ...
    expect(cubit.state.schedule.lastResortDelay, 7);
    cubit.setLastResortDelay(8);
    cubit.setProtection(BullVaultProtection.standard);
    expect(cubit.state.schedule.lastResortDelay, 8);
    cubit.setLastResortRecovery(false);
    expect(cubit.state.schedule.lastResortDelay, isNull);
    cubit.setProtection(BullVaultProtection.extra);
    expect(cubit.state.schedule.lastResortDelay, isNull);
    cubit.setLastResortRecovery(true);
    cubit.setInheritance(false);
    expect(cubit.state.schedule.lastResortDelay, isNull);
    await cubit.close();
  });
  ...
  for (final protection in BullVaultProtection.values) {
    for (final inheritance in [false, true]) {
      test(
        'navigates signer setup for ${protection.name}, inheritance: $inheritance',
        () async {
          final load = _MockLoadBullVaultOnboardingUsecase();
          final prepare = _MockPrepareBullVaultTimeReferenceUsecase();
          when(load.execute).thenAnswer(
          ...
          final cubit = _cubit(load: load, prepare: prepare);
          await cubit.load();
          cubit.setProtection(protection);
          await cubit.next();
          cubit.setInheritance(inheritance);
          await cubit.next();
          expect(cubit.state.step, BullVaultOnboardingStep.coldSigner);
          ...
          if (protection == BullVaultProtection.extra) {
            ...
            await cubit.acceptSecondColdKeyAndContinue(
              'second-cold-account-key',
            );
          }
          final lastColdStep = protection == BullVaultProtection.extra
          ...
          if (inheritance) {
            ...
            expect(cubit.state.canContinue, isFalse);
            cubit.back();
            expect(cubit.state.step, lastColdStep);
            await cubit.next();
          ...
          expect(
            cubit.state.step,
            inheritance ? BullVaultOnboardingStep.inheritance : lastColdStep,
          );
