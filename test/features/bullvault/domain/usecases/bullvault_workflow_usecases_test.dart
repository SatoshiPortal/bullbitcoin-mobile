...
void main() {
  ...
  test(
    'restores a pending renewal with its current setup requirements',
    () async {
  ...
  test(
    'loads the existing extra protection policy for renewal review',
    () async {
      final getDetails = _MockGetDetails();
      final resume = _MockResumeRenewal();
      ...
      final loaded = (result as Ok).value;
      expect(loaded.details, same(details));
      expect(loaded.renewal, isNull);
      expect(loaded.needsInitialSetup, isFalse);
    },
  );
