...
void main() {
  ...
  test('rejects practice recovery already active at chain time', () async {
    ...
    when(() => prepare.execute(isTestnet: true)).thenAnswer(
      (_) async => Ok(
        BullVaultTimeReference(
          deviceTime: reviewed.deviceTime,
          chainHeight: reviewed.chainHeight,
          medianTimePast:
              reviewed.deviceTimestamp + const Duration(hours: 8).inSeconds,
        ),
      ),
    );
    final result =
        await _usecase(
          ...
          BullVaultCreateRequest(
            label: 'Practice vault',
            protection: BullVaultProtection.standard,
            cold: BullVaultSignerRequest(
              input: deriveSignerKeys(testMnemonics[1]).xpub,
              device: SignerDeviceEntity.ledgerNanoX,
            ),
            secondCold: null,
            inheritance: null,
            schedule: BullVaultSchedule.standardWithoutInheritance.copyWith(
              unit: BullVaultScheduleUnit.hours,
            ),
            timeReference: reviewed,
          ),
        );
    expect((result as Err).failure, isA<BullVaultClockMismatchFailure>());
  ...
  test(
    ...
    () async {
      ...
      expect(accounts.claims, isEmpty);
      expect(accounts.reserved, isEmpty);
      verifyZeroInteractions(getDefaultSeed);
    },
  );
  ...
  test('accepts a nonzero external BIP48 account', () async {
    final repository = _TestBullVaultRepository();
    final externalAccount = deriveSignerKeysAtAccount(
      testMnemonics.first,
      account: 1,
    );
    final result = await _usecase(repository, getSettings).execute(
