...
void main() {
  group('RelativeFee.fromSatPerVbyte', () {
    test('1 sat/vByte = 250 sat/kwu (exact)', () {
      expect(
        NetworkFee.relativeFromSatPerVbyte(1.0),
        const RelativeFee(250),
      );
    });
    ...
    test('0.5 sat/vByte = 125 sat/kwu (exact)', () {
      ...
      expect(
        NetworkFee.relativeFromSatPerVbyte(0.5),
        const RelativeFee(125),
      );
    });

    test('0.1 sat/vByte = 25 sat/kwu (exact, the network minrelayfee floor)',
        () {
      expect(
        NetworkFee.relativeFromSatPerVbyte(0.1),
        const RelativeFee(25),
      );
    });
    ...
    test('0.25 sat/vByte rounds half-away-from-zero to 63 sat/kwu', () {
      ...
      expect(
        NetworkFee.relativeFromSatPerVbyte(0.25),
        const RelativeFee(63),
      );
    });
    ...
    test('0.001 sat/vByte rounds down to 0 sat/kwu (below precision)', () {
      ...
      expect(
        NetworkFee.relativeFromSatPerVbyte(0.001),
        const RelativeFee(0),
      );
    });
  ...
  group('NetworkFee.toAbsolute', () {
    test('AbsoluteFee passes through unchanged', () {
      expect(
        const AbsoluteFee(500).toAbsolute(200),
        const AbsoluteFee(500),
      );
    });
    ...
    test('RelativeFee × vsize → AbsoluteFee in sats', () {
      ...
      expect(
        const RelativeFee(125).toAbsolute(200),
        const AbsoluteFee(100),
      );
    });
    ...
    test('rounds half-up when satPerKwu × vsize is not divisible by 250', () {
      ...
      expect(
        const RelativeFee(125).toAbsolute(201),
        const AbsoluteFee(101),
      );
    });
  });
