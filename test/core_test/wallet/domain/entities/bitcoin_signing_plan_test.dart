...
void main() {
  ...
  test('evaluates threshold satisfaction for each input', () {
    ...
    for (final keyCount in [2, 3]) {
      final selected = signers.take(keyCount).toList();
      final remaining = BitcoinSigningPlan.fromPolicy(
        policy: _thresholdPolicy(
          threshold: 2,
          keyIds: selected.map((signer) => signer.descriptorKeys.single.id),
        ),
        signers: selected,
        inputKeychainsByOutpoint: const {
          'funding:0': BitcoinPolicyKeychain.external,
      ...
      expect(remaining.signersNeeded, keyCount == 2 ? 2 : 1);
    }
  });
