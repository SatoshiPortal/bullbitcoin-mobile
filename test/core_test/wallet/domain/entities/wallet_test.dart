...
void main() {
  ...
  test(
    'local seed references use canonical ownership and deduplicate keys',
    () {
      final protected = wallet(
        descriptorKeyCount: 2,
        localSeedFingerprint: 'DEADBEEF',
      );
      expect(protected.localSeedFingerprints, ['deadbeef']);
      expect(protected.singleLocalSeedFingerprint, 'deadbeef');
      expect(
        protected.descriptorKeys.map((key) => key.masterFingerprint).toSet(),
        {'00000000'},
      );
      expect(wallet(descriptorKeyCount: 2).localSeedFingerprints, ['00000000']);
      expect(
        wallet(signer: SignerEntity.remote).localSeedFingerprints,
        isEmpty,
      );
    },
  );
