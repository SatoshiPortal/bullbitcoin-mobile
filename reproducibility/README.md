...
Builds the base toolchain image with Rust, Flutter via FVM, Android SDK, and NDK. Also runs `flutter precache --android` to cache engine artifacts. This image rarely changes and is cached across builds.

### `../Containerfile.app` (root)
...
For container-to-container comparisons to be reproducible, both builds must use the exact same git commit. `SOURCE_DATE_EPOCH` is derived from `git log -1 --format=%ct`, so if two builds are from different commits they will embed different timestamps and the `.so` files will differ.
