...
- **`<Feature>Error` and `<Feature>Exception` are the same conformant surface** — one sealed family per feature, every variant with `toTranslated`, foreign errors mapped at the boundary. The suffix is a naming choice, not a design difference; both are accepted and neither warrants a rename. Prefer the suffix that reads naturally for the feature and keep it stable — churning `Error`↔`Exception` for its own sake is noise. A standalone one-off exception outside its feature's sealed family (no `toTranslated`) is the thing to fold in, regardless of suffix.
- **Rollback timing — one rule for every multi-step write flow.** Classify each failure point by the flow's **commitment point** (the first durable, externally-observable write — e.g. the manifest record, a server registration). **Pre-commitment** failures are **fatal and retryable**: roll back the partial local work and surface a typed error the caller can retry. **Post-commitment** failures are **best-effort**: log with the error/stack and continue — never roll back a durable/remote effect the app cannot un-happen, and never invent client-side rollback semantics for a request that may have reached the server (surface the uncertain state instead). Feature arch docs (e.g. `lightning_address`, `keychain_recovery`, `btcpay`) instantiate this rule for their flows; they reference it here rather than restating it.
