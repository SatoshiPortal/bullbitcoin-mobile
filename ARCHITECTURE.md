...
- `domain/<feature>_failure.dart` — the feature's sealed `Failure` family (Flutter-free); its `toTranslated(context)` lives in a `presentation/<feature>_failure_l10n.dart` extension, never on the failure itself.
...
<feature>/
  ...
  presentation/       # <feature>_bloc.dart + state + event + <feature>_failure_l10n.dart (toTranslated)
  ui/
    screens/
    widgets/
...
3. **Presentation.** `<Feature>Bloc`/`Cubit` + sealed state/event, plus the `<feature>_failure_l10n.dart` extension exposing `toTranslated(BuildContext)` (`presentation/`). The bloc stores the `<Feature>Failure` in state; it calls use-cases only — never a repository or datasource.
...
**Current state.** The app remains at the repo root and is declared as a workspace package through `useRootAsPackage: true`. The root `pubspec.yaml` `workspace:` key lists 11 members (checked 2026-09-02): `packages/bull_ui` owns the design system, `packages/bull_ui_catalogue` is its dev-only Widgetbook, `packages/primitives` owns shared pure-Dart value types and results, `packages/wallet_transaction_sync` owns an SDK-independent public contract backed internally by BDK/LWK local-source adapters, `packages/notifications` owns local-notification delivery and its durable outbox, `packages/background_tasks` owns headless task dispatch and wallet-sync orchestration, `packages/bull_payjoin` owns the pure-Dart Payjoin contract, engine, and persistence, the Flutter-dependent `packages/bull_tor` owns embedded Tor, `packages/screen_privacy` owns screenshot/recording-capture blocking, `packages/bull_logger` owns logging and diagnostic infrastructure, and `features/logs` owns the user-facing diagnostic log viewer.

### Wallet transaction synchronization package

`packages/wallet_transaction_sync` is the audit boundary for what the application claims to know about Bitcoin and Liquid transaction history, how that observation was obtained, and how fresh it is. BDK remains the durable Bitcoin authority and LWK remains the durable Liquid authority; the package publishes an immutable normalized snapshot in memory and deliberately adds no second transaction database. Labels, swaps, exchange orders, presentation mapping, spend construction, signing, key material, and UI remain outside the package. Its package-local contract and migration state are documented in [`packages/wallet_transaction_sync/README.md`](packages/wallet_transaction_sync/README.md).

The package follows the package form of the project layer model: public domain entities, sealed failures, source registrations/configurations, ports, thin use-cases, and `WalletTransactionSyncFacade` are exported from `lib/wallet_transaction_sync.dart`; repositories, the in-memory snapshot store, and the BDK/LWK adapters stay under `lib/src/data/`. SDK types are confined to those adapters and never cross the repository or facade boundary. The public operations are `synchronizeWallet`, `discoverWalletHistory`, `refreshLocalSnapshot`, `lookupLocal`, `listLocal`, `watchWalletState`, `deleteWallet`, and `dispose`.

The three read/sync concepts are intentionally separate. `synchronizeWallet` performs network synchronization and persists authoritative SDK state; `discoverWalletHistory` performs explicit restoration/history discovery; `refreshLocalSnapshot` reconstructs from already-persisted SDK state without network access. `lookupLocal` and `listLocal` read only the current in-memory revision, and `watchWalletState` observes state/revision transitions but never initiates synchronization. A successful source observation is validated and fingerprinted before one complete snapshot replaces the previous revision; identical content keeps its revision, while any modeled transaction, position, evidence, tip, or completeness change creates a new revision and expires pagination cursors.

`WalletSourceRegistration` binds `(walletId, chain, network)` to a source kind and a non-reversible fingerprint of wallet identity. Descriptors are supplied ephemerally to the source adapter and are never part of a transaction snapshot, diagnostic string, or log; metadata implementations may persist receipts, timestamps, deletion phases, and content fingerprints, but must not serialize the registration's descriptor-bearing configuration. Recoverable SDK exceptions are mapped once into `WalletTransactionSyncFailure`; source strings are not public error messages.

A durable `WalletSourceOperationCoordinator` serializes every operation that opens, reads, mutates, persists, signs with, or deletes the same BDK/LWK state. Its key is `(walletId, chain, network)`, persisted only as a hash; each Flutter isolate owns a fresh coordinator instance, while per-key SQLite mutex files, monotone generations, random owner tokens, and foreground-before-background admission provide one logical lock across foreground and WorkManager isolates. Sessions become unusable after their operation, caller timeouts never release active work, and successful deletion retires the key so an operation that captured stale metadata cannot reopen deleted state. Explicit successful discovery may reactivate a retired key. The foreground singleton is injected into legacy wallet repositories and Payjoin adapters; background package operations use their isolate-local instance against the same durable coordination directory.

The BDK adapter owns a short-lived wallet/persister pair, disposes the wallet before its persister, distinguishes routine revealed-script sync from full discovery, applies and persists one selected Electrum update, and extracts canonical positions with block identity when available. The LWK adapter owns and disposes a short-lived public watch-only wallet reference, uses one `transactionsProjection(includeUnblindingData: false)` call for a coherent complete snapshot, preserves nullable vin/vout positions through original indexes plus slot counts, retains asset and ownership metadata, and never requests an unblinding URL for package history. LWK height/time becomes source-reported confirmation evidence only; the package does not invent a block hash, tip, confirmation count, or mempool proof.

Migration status matters: both source adapters and the facade exist and are independently testable, the durable coordinator guards legacy foreground consumers, and WorkManager now uses `WalletTransactionSyncFacade` for watch-only background synchronization and incoming-payment detection. Legacy repositories still feed the foreground transaction UI. Shadow parity, publication of immutable upstream LWK/SDK commits, and separately authorized rollback-capable cutovers must precede migration of details, wallet lists, or aggregate lists.

### Background tasks and notifications packages

`packages/background_tasks` is a headless orchestration package: it knows task identifiers, retry/permanent outcomes, WorkManager dispatch, and how to reconcile wallet snapshots with notifications, but it imports no app feature or shell implementation. The root shell owns the top-level `@pragma('vm:entry-point')` callback and creates a fresh dependency graph for every WorkManager isolate. That graph uses public watch-only descriptors only, resolves Electrum through the existing privacy/fallback port, assigns background coordinator priority, writes to the separate background log, and closes wallet facades, package SQLite stores, Drift, and the isolate-local locator before returning. Bitcoin and Liquid sync require network connectivity; log pruning does not. Obsolete swap work is cancelled by unique name while an already-fired legacy task remains a successful no-op.

`packages/notifications` owns a durable SQLite outbox and the `flutter_local_notifications` gateway. A topic's first complete wallet snapshot establishes a silent baseline; later reconciliations enqueue only transactions whose direction is exactly incoming and which are not self-transfers. Topic and event keys are versioned SHA-256 values, platform notification IDs are deterministic and collision-safe, and persisted responses contain only the destination payload. Initialization never prompts. The shell asks for notification permission only after startup has established that no PIN is required or after PIN unlock succeeds, then consumes the deferred destination and routes it through `AppRouter`; the only current destination is `WalletRoute.walletHome`.
