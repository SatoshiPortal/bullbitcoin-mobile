...
One process-wide `WalletSourceOperationCoordinator` serializes every operation that opens, reads, mutates, persists, signs with, or deletes the same BDK/LWK state. Its key is `(walletId, chain, network)`, sessions become unusable after their operation, timed-out work retains ownership until it actually finishes, and successful deletion retires the key so an operation that captured stale metadata cannot reopen deleted state. Explicit successful discovery may reactivate a retired key. The same coordinator instance is already injected into legacy wallet repositories and Payjoin adapters, which prevents mixed package/legacy access from racing while migration is incomplete.

The BDK adapter owns a short-lived wallet/persister pair, disposes the wallet before its persister, distinguishes routine revealed-script sync from full discovery, applies and persists one selected Electrum update, and extracts canonical positions with block identity when available. The LWK adapter owns a short-lived public watch-only wallet reference, uses one `transactionsProjection(includeUnblindingData: false)` call for a coherent complete snapshot, preserves nullable vin/vout positions through original indexes plus slot counts, retains asset and ownership metadata, and never requests an unblinding URL for package history. LWK height/time becomes source-reported confirmation evidence only; the package does not invent a block hash, tip, confirmation count, or mempool proof.

Migration status matters: both source adapters and the facade exist and are independently testable, and the coordinator already guards legacy consumers, but the application composition root does not yet use `WalletTransactionSyncFacade` as the production transaction-history owner. Legacy repositories still feed the UI. Shadow parity, publication of immutable upstream LWK/SDK commits, and separately authorized rollback-capable cutovers must precede migration of details, wallet lists, or aggregate lists.
