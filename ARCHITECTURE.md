...
**Current state.** The app remains at the repo root and is declared as a workspace package through `useRootAsPackage: true`. The root `pubspec.yaml` `workspace:` key lists 9 members (checked 2026-08-27): `packages/bull_ui` owns the design system, `packages/bull_ui_catalogue` is its dev-only Widgetbook, `packages/primitives` owns shared pure-Dart value types and results, `packages/bull_payjoin` owns the pure-Dart Payjoin contract, engine, and persistence, the Flutter-dependent `packages/bull_tor` owns embedded Tor, `packages/screen_privacy` owns screenshot/recording-capture blocking, `packages/bull_logger` owns logging and diagnostic infrastructure, `features/logs` owns the user-facing diagnostic log viewer, and `features/recoverbull` is the first extracted user-facing feature package. RecoverBull establishes the shell-adapter pattern: the feature owns its UI, domain, persistence, routes, and public contract while the root app supplies wallet, settings, logging, and Tor capabilities at composition time.
...
This is a slow, deliberate migration, but its direction is explicit: more independently owned modules will move into `features/` and `packages/` as scoped extraction work is approved. Until a module is extracted, it stays in `lib/` and follows the same rules it does today. Do not move code opportunistically — extraction is its own scoped, reviewed change with the security/bitcoin/build implications considered per module — but once a module is extracted, fix integration through its public API and shell adapters rather than moving ownership back into `lib/` or creating a duplicate implementation.
