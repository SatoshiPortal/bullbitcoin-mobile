...
**Package (shared foundation or infrastructure — `packages/<name>`):** an app-internal module consumed by features. Most packages contain the `domain/` + `data/` half of the same shape, **with no `presentation/` and no `ui/`**, and remain pure Dart. An infrastructure package may depend on Flutter when the capability it encapsulates inherently requires a Flutter plugin or platform lifecycle, as `tor` does for Onion/FRB and `path_provider`. It exposes repository interfaces, domain types, shared use-cases, and any application-level infrastructure facade through its public API (`lib/<name>.dart`); everything else stays under `lib/src/`.
...
**Current state (incremental extraction).** The app remains at the workspace root via `useRootAsPackage: true`, alongside the `bull_ui`, `bull_ui_catalogue`, and Flutter-dependent `tor` workspace members. `features/` remains the reserved home for future feature extraction.
...
**Tor package (`packages/tor`).** Tor is a Bull Mobile-specific Flutter infrastructure package; its name is `tor` because the repository already supplies the Bull context. The feature-facing `tor.dart` library publishes validated routes, sealed connection states, typed failures, repository/use-case APIs, and the HTTP-client factory consumed by RecoverBull. The narrower shell-facing `tor_adapter.dart` publishes composition, lifecycle, logging, and test seams. Internally the package owns Onion/Arti initialization, state/cache paths, mobile dormancy, the embedded SOCKS listener adapter, and external Orbot SOCKS verification. The root shell only supplies application logging callbacks, mounts the package locator, and starts/disposes the published lifecycle controller. Bull's embedded Onion client serves RecoverBull and Bitcoin Electrum `.onion` connections through isolated sessions, starting when either concern requires a route. The user-managed Orbot SOCKS endpoint remains a separate, explicit override for Bitcoin Electrum `.onion` servers. Other application traffic is unchanged; users who want broader routing configure Orbot's VPN mode outside Bull.
