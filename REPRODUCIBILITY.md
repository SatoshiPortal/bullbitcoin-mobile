...
### `Dockerfile` and `Dockerfile.build`

`Dockerfile` sets up the build toolchain (Rust, Flutter via FVM, Android SDK, Gradle) and also installs `apktool` and `bundletool` - used by `scripts/verify_build.sh` to decode APKs and to generate splits from an AAB.

`Dockerfile.build` extends the base image: it copies the repo in, runs `pub get` / `build_runner` / `gen-l10n`, configures a keystore and Gradle, then runs `flutter build`. Two environment variables are set at build time to eliminate sources of non-determinism:
...
3. Builds the app from the current repo checkout using `Dockerfile.build`
4. Extracts the built artifact from the image
...
To just produce a build artifact without verification, use `make build` (see the makefile for `MODE` and `FORMAT` options).
