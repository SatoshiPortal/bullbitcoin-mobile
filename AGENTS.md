...
The repo is migrating incrementally to a [melos](https://melos.invertase.dev/) pub-workspace. The Flutter app remains at the repo root through `useRootAsPackage: true`. Current `workspace:` members (checked 2026-08-25, see ARCHITECTURE.md "Monorepo Migration" for what each one owns): `bull_ui`, `bull_ui_catalogue`, `primitives`, `bull_payjoin`, `bull_tor`, `screen_privacy`. The makefile remains the canonical entry point across the workspace.
